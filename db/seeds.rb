...
sample["creators"].each do |creator|
  ...
  creators[creator["youtube_name"]] = Creator.create! creator.slice("youtube_name", "channel_url", "video_url", "nb_followers", "description", "is_showcased", "").merge(user: user).merge(batch: batch)
end
